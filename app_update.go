...
func (AppUpdate) Check(flags *Flags) error {
	if !flags.LoggedIn {
		return fmt.Errorf("steamcmd: cannot app_update before login")
	}
...
func (c AppUpdate) Args() ([]string, error) {
	if c.AppID == 0 {
		return nil, fmt.Errorf("steamcmd: app_update requires app ID")
	}
	...
	if c.Beta != "" && c.Beta != DefaultBranch {
		if c.BetaPassword == "" {
			return nil, fmt.Errorf("steamcmd: app_update -betapassword is required with -beta")
		}

		args = append(args, "-beta", c.Beta, "-betapassword", c.BetaPassword)
	} else if c.BetaPassword != "" {
		return nil, fmt.Errorf("steamcmd: app_update -betapassword is prohibited without -beta")
	}
